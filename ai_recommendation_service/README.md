...
Khi chạy `docker compose up --build`, `recommender-service` sẽ tự:

- migrate database
- import `dataset/dataset.csv` vào `tracking_eventlog` (nếu chưa có dữ liệu)
- train BiLSTM và nạp lại model
